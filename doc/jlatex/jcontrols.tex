...
{\em obj}によって示されるコンディションをsignalizeする。最初にマッチングされたhandlerの返り値を返す。マッチングされずにどのhandlerも呼ばれない場合はnilを返す。
{\em obj}はコンディションインスタンスか、 {\em init-args}によって初期化されるコンディションクラスである。}

\funcdesc{invoke-next-handler}{obj}{
  コンディションハンドラーの中から呼ばれた場合、コンディションインタンス{\em obj}と次にマッチングされたhandlerの返り値を返す。マッチングされずにどのhandlerも呼ばれない場合はnilを返す。}
