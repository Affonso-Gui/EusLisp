...
pointer mod;
{
  ...
  QUOTE=defspecial(ctx,"QUOTE",mod,quote,"(thing)");
  QEVAL=defun(ctx,"EVAL",mod,EVAL);
  defun(ctx,"APPLY",mod,APPLY);
  defun(ctx,"FUNCALL",mod,FUNCALL);
  QPROGN=defspecial(ctx,"PROGN",mod,progn,"(&rest forms)");
  defun(ctx,"PROG1",mod,PROG1); 
  FUNCTION=defspecial(ctx,"FUNCTION",mod,FUNCTION_CLOSURE,"(thing)");
  defun(ctx,"MAPC",mod,MAPC);
  defun(ctx,"MAPCAR",mod,MAPCAR);
  defun(ctx,"MAPCAN",mod,MAPCAN);
  defspecial(ctx,"SETQ",mod,SETQ,"(&rest things)");
  QIF=defspecial(ctx,"IF",mod,IF,"(test then &optional else)");
  defmacro(ctx,"WHEN",mod,WHEN);
  defspecial(ctx,"COND",mod,COND,"(&rest args)");
  defspecial(ctx,"WHILE",mod,WHILE,"(test &rest forms)");
  defspecial(ctx,"LET",mod,PARLET,"(bindings &rest body)");
  defspecial(ctx,"LET*",mod,SEQLET,"(bindings &rest body)");
  defspecial(ctx,"UNWIND-PROTECT",mod,UNWINDPROTECT,"(protected &rest cleanup)");
  defspecial(ctx,"CATCH",mod,CATCH,"(tag &rest body)");
  defspecial(ctx,"THROW",mod,THROW,"(tag result)");
  defspecial(ctx,"FLET",mod,FLET,"(definitions &rest body)");
  defspecial(ctx,"LABELS",mod,LABELS,"(definitions &rest body)");
  defspecial(ctx,"BLOCK",mod,BLOCK,"(name &rest forms)");
  QRETFROM=defspecial(ctx,"RETURN-FROM",mod,RETFROM,"(name &optional value)");
  defmacro(ctx,"RETURN",mod,RETURN);
  defspecial(ctx,"TAGBODY",mod,TAGBODY,"(&rest statements)");
  defspecial(ctx,"GO",mod,GO,"(tag)");
  defun(ctx,"RESET",mod,RESET);
  defun(ctx,"EVALHOOK",mod,EVALHOOK);
  defun(ctx,"MACROEXPAND2",mod,MACEXPAND2);
  defspecial(ctx,"EVAL-WHEN",mod,EVALWHEN,"(situations &rest forms)");
  defspecial(ctx,"THE",mod,THE,"(value-type form)");
  QAND=defspecial(ctx,"AND",mod,AND,"(&rest args)");
  QOR=defspecial(ctx,"OR",mod,OR,"(&rest args)");
  defun(ctx,"PROCLAIM",mod,PROCLAIM);
  defspecial(ctx,"DECLARE",mod,DECLARE,NULL);
